...
import pandas as pd  # (ok to keep even if unused)
import cv2
...
def window1D(npt=500, filter_frac=0.25):
    ...
    Slightly more general than scipy.signal.tukey (special case: filter_frac=0.5)
    """
    ...
    ifw = int(fw)
    filt = np.ones(npt)
    filt[0:ifw] = (1.0 - np.cos(np.pi * np.arange(ifw) / fw)) / 2.0
    filt[-1:(-ifw - 1):-1] = (1.0 - np.cos(np.pi * np.arange(ifw) / fw)) / 2.0
    return filt
...
def window2D_sym(npt=[500, 500], filter_frac=0.25):
    """Symmetric window centered in a possibly rectangular array."""
    min_npt = np.min(npt)
    central_window = window2D(min_npt, filter_frac=filter_frac)
    if np.array([npt]).size == 1 or npt[0] == npt[1]:
        return central_window
    window = np.zeros(npt)
    window[(npt[0] - min_npt)//2:(npt[0] - min_npt)//2 + min_npt,
           (npt[1] - min_npt)//2:(npt[1] - min_npt)//2 + min_npt] = central_window
    return window

def window_image(im, filter_frac=0.25, remove_mean=False, shift=False):
    """Apply a cosine-edged window; optional mean removal and fft-shift alignment."""
    if len(im.shape) != 2:
        _log.critical("Non grayscale image passed to window function")

    im2 = im.copy()
    filt = window2D(im.shape, filter_frac=filter_frac)
    if shift:
        im2 = np.fft.fftshift(im2)

    if remove_mean:
        im2 = filt * (im2 - np.mean(im2))
    else:
        im2 = filt * im2
...
def load_image(
    ...
    """
    Load an image (BGR), convert to grayscale, optional crop/resize & window.
    Returns filtered image, and if return_unfiltered=True also returns raw gray.
    """
    filename = str(filename)
    if not Path(filename).exists():
        raise FileNotFoundError(f"Image path does not exist: {filename}")
    ...
    if downsample != 1:
        im = cv2.resize(im, (im.shape[1] // downsample, im.shape[0] // downsample),
...
def pad_image(im, pad_factor=2, filter_frac=0.25, window=True, shift=True):
    """Pad an image, possibly applying a window first."""
    npts = im.shape[0]
    ...
    im_pad[(npts_pad//2 - npts//2):(npts_pad//2 + npts//2),
           (npts_pad//2 - npts//2):(npts_pad//2 + npts//2)] = im2
    if shift:
        im_pad = np.fft.ifftshift(im_pad)
    return im_pad

def centre_crop(im, npt_crop, shift=False, offset_yx=[0, 0]):
    """Center crop with optional fft-shift alignment."""
    max_crop = int(np.min(im.shape[0:2]))
    if npt_crop is None:
        npt_crop = max_crop
    else:
        npt_crop = min(int(npt_crop), max_crop)
...
def field_positions(imshape, ifieldsXY, field_extent=[-1, 1, -1, 1]):
    """Map pixel indices -> normalized field positions."""
...
def field_indices(imshape, fieldsXY, field_extent=[-1, 1, -1, 1]):
    """Map normalized field positions -> pixel indices (and cleaned fieldsXY)."""
    nxy = np.array(imshape[1::-1])
...
def field_corners(imshape, fieldsXY, field_extent=[-1, 1, -1, 1], return_cent=False):
    """
    Select the TL/TR/BL/BR (and optional C) positions from a set of fields.
    Returns pixel indices, normalized positions, and labels.
    """
    ...
    if return_cent:
        ...
        argC = np.argmin(
            np.linalg.norm(
                np.array(fieldsXY) - np.array([np.mean(field_extent[0:2]), np.mean(field_extent[2:4])]),
                axis=1
            )
        )
...
def plot_hypertarget_overview(
    ...
    plt.imshow(im, cmap="gray", extent=extent, aspect=im.shape[0] / im.shape[1])  # extent L,R,B,T
    ...
    if plot_patches:
        ...
        ifieldsXY, _ = field_indices(im.shape, fieldsXY, field_extent=field_extent)
...
class HyperTarget:
    """
    Finds coordinates for:
      - slant edge patches
      - textured patches (and their corners)
      - flat patches
    """
    ...
    def __init__(
        ...
        """
        Quick detector based on downsampled matched filtering for the target squares.
        Assumes fabricated squares of size `target_square_npix` on the target.
        """
        self.field_extent = field_extent
        self.imshape = im_gray.shape

        # Predicted square size in the downsampled image
        ...
        # Transform & matched filter with a negative square tile
        im2 = np.abs(im - np.mean(im))
        im2 = im2 - np.mean(im2)
        tile = -np.ones((image_square_npix_ds, image_square_npix_ds))
        conv = scipy.signal.convolve2d(im2, tile)

        # Local maxima -> candidate centers
        coordinates = (
            peak_local_max(
                conv,
                min_distance=max(1, image_square_npix_ds // 2),
        ...
        # Delaunay edges -> slant angle estimate
        ...
        # Build four rims from the grid to find slant-edge fields
        edge_fieldsXY = []
        ...
        self.slant_ifieldsXY = field_indices(imshape=im.shape, fieldsXY=self.slant_fieldsXY, field_extent=field_extent)[0]

        # Textured fields & corners
        ...
        # Flat fields (local minima of the convolution)
        flat_coordinates = (
            peak_local_max(
                -conv,
                min_distance=max(1, image_square_npix_ds // 2),
        ...
        # Optional plots
        if plot_slant:
            dxy = max(1, 2 * image_square_npix_ds // 5)
            _, fieldsXY = self.slant_ifieldsXY, self.slant_fieldsXY
            plot_hypertarget_overview(im, fieldsXY, dxy=dxy, title=self.title,
                                      show_corners=False, plot_patches=plot_patches, field_extent=field_extent)

        if plot_text:
            dxy = max(1, image_square_npix_ds)
            title = "FOCUS FIELDS/CORNER %s" % self.title
            _, fieldsXY = self.text_ifieldsXY, self.text_fieldsXY
            plot_hypertarget_overview(im, fieldsXY, dxy=dxy, title=title,
                                      show_corners=True, plot_patches=plot_patches, field_extent=field_extent)

        if plot_flat:
            dxy = max(1, image_square_npix_ds // 2)
            title = "FLAT TEXTURES %s" % self.title
            _, fieldsXY = self.flat_ifieldsXY, self.flat_fieldsXY
            plot_hypertarget_overview(im, fieldsXY, dxy=dxy, title=title,
                                      show_corners=False, plot_patches=plot_patches, field_extent=field_extent)

        # Map coords back to full-res image space
    ...
    def getFieldIndices(self, fieldsXY, im_gray=None):
        imshape = self.imshape if im_gray is None else im_gray.shape
        return field_indices(imshape, fieldsXY)
    ...
    def getDarkNoiseEstimate(self, im_gray):
        ...
        for ifieldXY in self.flat_ifieldsXY:
            ...
            varsum += np.var(patch)
        return varsum / max(1, len(self.flat_ifieldsXY))

    def getDarkBrightNoiseEstimates(self, im_gray):
        darkvar = brightvar = 0.0
        darkmean = brightmean = 0.0
        nfields = max(1, len(self.flat_ifieldsXY))
...
# ------------------------------------------------------------------
# Demo / self-test only runs when you execute this file directly
# ------------------------------------------------------------------
if __name__ == "__main__":
    import argparse
    p = argparse.ArgumentParser(description="Quick HyperTarget demo (for debugging).")
    p.add_argument("image", help="Path to a test image (PNG/JPG).")
    p.add_argument("--plot", action="store_true", help="Show slant/text/flat overview plots.")
    args = p.parse_args()

    try:
        filt_im, im = load_image(args.image, return_unfiltered=True)
    except Exception as e:
        print(f"[ERROR] {e}")
        raise SystemExit(1)

    ht = HyperTarget(im, plot_slant=args.plot, plot_text=args.plot, plot_flat=False, plot_patches=args.plot)
    print(ht)
